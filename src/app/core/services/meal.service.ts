...
export class MealService {
  ...
  updateMeal(
    mealId: number,
    request: UpdateMealDTO,
  ): Observable<MealResponseDTO> {
    return this.http.put<MealResponseDTO>(
      `${environment.apiUrl}/meal/${mealId}`,
      request,
    );
  }
  ...
  addMealPortion(request: AddMealPortionDTO): Observable<FoodEntryDto[]> {
    return this.http.post<FoodEntryDto[]>(
      `${environment.apiUrl}/meal/portion`,
      request,
    );
  }

  getMealShareId(mealId: number): Observable<string> {
    return this.http.get(`${environment.apiUrl}/meal/${mealId}/share`, {
      responseType: 'text',
    });
  }
  ...
  addMealByShareId(shareId: string): Observable<MealResponseDTO> {
    return this.http.post<MealResponseDTO>(
      `${environment.apiUrl}/meal/${shareId}/share`,
      {},
    );
  }
}
